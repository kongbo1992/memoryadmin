...
<div class="tb-module-form">
    ...
    <div class="col-sm-12" >
        <?= $form->field($model, 'content')->textarea(['rows' => 17]) ?>
    </div>
    <div class="col-sm-12">
        <?= $form->field($model, 'type')->dropDownList(['0' => '照片墙','1' => '轮播']) ?>
    </div>
    <div class="col-sm-12">
        <?=  $form->field($model, 'imgurl')->hiddenInput()->label('相册封面')?>
        <?= $form->field($model, 'qiniu_img_file')->widget(FileInput::className(),[
            ...
            'pluginEvents' => [
                ...
                "fileuploaded" => "function (event, data, id, index) {
                        $('input').each(function(){
                            if(this.name == 'TbModule[imgurl]' && this.type!='file'){
